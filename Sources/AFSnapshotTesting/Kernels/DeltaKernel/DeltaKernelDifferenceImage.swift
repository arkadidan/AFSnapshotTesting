//
// DeltaKernelDifferenceImage.swift
// AFSnapshotTesting
//
// Created by Afanasy Koryakin on 27.03.2025.
...
class DeltaKernelDifferenceImage: Kernel {
    init(with configuration: Kernel.Configuration) throws {
        try super.init(with: configuration, function: "deltaKernelTextureRecord")
    }

    func differenceImage(lhs: CGImage, rhs: CGImage, tollerance deltaE: Float, color: MismatchColor) throws -> CGImage {
        ...
        var tolleranceValue = deltaE
        let tolleranceKernelBuffer = device.makeBuffer(bytes: &tolleranceValue, length: MemoryLayout<Float>.size, options: [])
        computeEncoder.setBuffer(tolleranceKernelBuffer, offset: 0, index: 0)
